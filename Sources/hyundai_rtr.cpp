...
Hyundai_rtr::Hyundai_rtr(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::Hyundai_rtr)
{
    ui->setupUi(this);
    ...
    connect(transmitTimer, &QTimer::timeout, this, &Hyundai_rtr::sendCANFrame);
...
void Hyundai_rtr::updateTable(const QList<CANMessageTransmit>& messages)
{
...
void Hyundai_rtr::updateTableReceive(const QList<CANMessageReceive>& messages)
{
...
void Hyundai_rtr::startBackgroundProcess(const QString &command)
{
...
void Hyundai_rtr::on_buttonReturn_clicked()
{
    Hynndai *hy;
    hide();
    hy = new Hynndai(this);
...
void Hyundai_rtr::on_btCANInterface_clicked()
{
...
void Hyundai_rtr::on_BtnSetRequest_clicked()
{
...
void Hyundai_rtr::on_pushButton_clicked(bool checked)
{
...
QList<CANMessageReceive> Hyundai_rtr::filterCANMessages(const QList<CANMessageReceive>& messages) {
...
void Hyundai_rtr::receiveCANFrame() {
    if (canDevice && canDevice->state() == QCanBusDevice::ConnectedState) {
...
void Hyundai_rtr::Delay(int milliseconds) {
    QEventLoop loop;
    QTimer::singleShot(milliseconds, &loop, &QEventLoop::quit);
    loop.exec();
}

void Hyundai_rtr::on_pushButton_2_clicked()
{
...
void Hyundai_rtr::on_buttonSave_clicked()
{
...
void Hyundai_rtr::on_buttonOpen_clicked()
{
...
void Hyundai_rtr::on_buttonHome_clicked()
{
    hide();
    MainWindow *mw ;
