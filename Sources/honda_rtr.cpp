...
Honda_rtr::Honda_rtr(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::Honda_rtr)
{
    ui->setupUi(this);
    ...
    connect(transmitTimer, &QTimer::timeout, this, &Honda_rtr::sendCANFrame);
...
void Honda_rtr::updateTable(const QList<CANMessageTransmit>& messages)
{
...
void Honda_rtr::updateTableReceive(const QList<CANMessageReceive>& messages)
{
...
void Honda_rtr::startBackgroundProcess(const QString &command)
{
...
void Honda_rtr::on_buttonReturn_clicked()
{
    hide();
    Honda1 *hd1 ;
    hd1 = new Honda1(this);
...
void Honda_rtr::on_btCANInterface_clicked()
{
...
void Honda_rtr::on_BtnSetRequest_clicked()
{
...
void Honda_rtr::on_pushButton_clicked(bool checked)
{
...
QList<CANMessageReceive> Honda_rtr::filterCANMessages(const QList<CANMessageReceive>& messages) {
...
void Honda_rtr::receiveCANFrame() {
    if (canDevice && canDevice->state() == QCanBusDevice::ConnectedState) {
...
void Honda_rtr::Delay(int milliseconds) {
    QEventLoop loop;
    QTimer::singleShot(milliseconds, &loop, &QEventLoop::quit);
    loop.exec();
}

void Honda_rtr::on_pushButton_2_clicked()
{
...
void Honda_rtr::on_buttonSave_clicked()
{
...
void Honda_rtr::on_buttonOpen_clicked()
{
...
Honda_rtr::~Honda_rtr()
{
    delete ui;  // Giải phóng bộ nhớ cho `ui`
}



void Honda_rtr::on_buttonHome_clicked()
{
    hide();
    MainWindow *mw ;
