...
#include "ui_mazda_decode.h"
#include <QVBoxLayout>
MainWindow *MainD;
Mazda *Md;
Mazda_Decode::Mazda_Decode(QWidget *parent)
    : QDialog(parent)
    ,_serialPort(nullptr)
    , ui(new Ui::Mazda_Decode)

{
    ...
    QList<DecodeFrame> infor = {
                                         {"0x202", "Engine speed", "RPM", " ", " "},
                                         {"0x202", "Vehicle speed", "Km/h", " ", " "},
                                         {"0x215", "Wheel speed (FL)", "Km/h", " ", " "},
                                         {"0x215", "Wheel speed (RL)", "Km/h", " ", " "},
                                         {"0x215", "Wheel speed (FR)", "Km/h", " ", " "},
                                         {"0x215", "Wheel speed (RR)", "Km/h", " ", " "},
                                         {"0x091", "Right turn switch", "ON/OFF", " ", " "},
                                         {"0x091 ", "Left turn switch", "ON/OFF", " ", " "},
                                         {"0x091 ", "Hazard", "ON/OFF", " ", " "},
    ...
    connect(ui->buttonVehicleSpeed, &QPushButton::clicked, this, &Mazda_Decode::on_buttonVehicleSpeed_clicked);
    connect(ui->buttonEngineSpeed, &QPushButton::clicked, this, &Mazda_Decode::on_buttonEngineSpeed_clicked);
    connect(ui->buttonWspeedFL, &QPushButton::clicked, this, &Mazda_Decode::on_buttonWspeedFL_clicked);
    connect(ui->buttonWspeedFR, &QPushButton::clicked, this, &Mazda_Decode::on_buttonWspeedFR_clicked);
    connect(ui->buttonWspeedRL, &QPushButton::clicked, this, &Mazda_Decode::on_buttonWspeedRL_clicked);
    connect(ui->buttonWspeedRR, &QPushButton::clicked, this, &Mazda_Decode::on_buttonWspeedRR_clicked);

    // Initialize the timer for saving data
    saveTimer = new QTimer(this);
    connect(saveTimer, &QTimer::timeout, this, &Mazda_Decode::saveDataContinuously);

    // Connect start and stop buttons
    connect(ui->buttonSaveStart, &QPushButton::clicked, this, &Mazda_Decode::on_buttonSaveStart_clicked);
    connect(ui->buttonSaveStop, &QPushButton::clicked, this, &Mazda_Decode::on_buttonSaveStop_clicked);
}

Mazda_Decode::~Mazda_Decode()
{
    delete ui;
}

void Mazda_Decode::updateTable(const QList<DecodeFrame>& messages)
{
...
void Mazda_Decode::on_buttonReturn_2_clicked()
{
    hide();
    Md = new Mazda(this);
    Md->show();
...
void Mazda_Decode::on_buttonHome_2_clicked()
{
    hide();
    MainD = new MainWindow(this);
    MainD->show();
    if (_serialPort != nullptr && _serialPort->isOpen()) {
        _serialPort->close();
        qDebug() << "Port Disconnected: COM port has been disconnected.";
...
void Mazda_Decode::on_btnPortsInfo_clicked()
{
...
void Mazda_Decode::on_buttonOpen_2_clicked()
{
    ...
    connect(_serialPort, &QSerialPort::readyRead, this, &Mazda_Decode::handleSerialData);
}

void Mazda_Decode::handleSerialData() {
...
void Mazda_Decode::updateTableValue(const QString &specification, const QString &rawData, const QString &lastValue) {
...
void Mazda_Decode::on_buttonVehicleSpeed_clicked()
{
    if (!graphWindow->isVisible()) {
...
void Mazda_Decode::on_buttonEngineSpeed_clicked()
{
    if (!graphWindow->isVisible()) {
        graphWindow->showEngineSpeedChart();
    }
}
void Mazda_Decode::on_buttonWspeedRR_clicked()
{
    if (!graphWindow->isVisible()) {
...
void Mazda_Decode::on_buttonWspeedRL_clicked()
{
    if (!graphWindow->isVisible()) {
...
void Mazda_Decode::on_buttonWspeedFL_clicked()
{
    if (!graphWindow->isVisible()) {
...
void Mazda_Decode::on_buttonWspeedFR_clicked()
{
    if (!graphWindow->isVisible()) {
...
void Mazda_Decode::on_buttonSaveStart_clicked()
{
...
void Mazda_Decode::saveDataContinuously() {
    QString dataRow;
...
void Mazda_Decode::on_buttonSaveStop_clicked()
{
    if (saveTimer->isActive()) {
...
void Mazda_Decode::saveDataToFile(const QString &filePath) {
    QFile file(filePath);
...
void Mazda_Decode::startPulseview()
{
...
void Mazda_Decode::on_pushButton_clicked()
{
    startPulseview();
}
